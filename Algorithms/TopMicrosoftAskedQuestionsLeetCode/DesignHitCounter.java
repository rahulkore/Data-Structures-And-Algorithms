...
import java.util.*;
/**Design a hit counter which counts the number of hits received in the past 5 minutes (i.e., 
...
int getHits(int timestamp) Returns the number of hits in the past 5 minutes from timestamp (i.e.,
 the past 300 seconds).
...
public class DesignHitCounter {
    ...
    public int getHits(int timestamp) {
        int res = 0;
        int end = timestamp - 300;
        while(timestamp > end){
            res += map.getOrDefault(timestamp, 0);
            timestamp--;
        }
        return res;
    }
