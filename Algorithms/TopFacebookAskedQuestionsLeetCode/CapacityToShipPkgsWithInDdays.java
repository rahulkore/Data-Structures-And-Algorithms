...
2nd day: 2, 4
3rd day: 1, 4 */
public class CapacityToShipPkgsWithInDdays {

    public int countDays(int[] nums,int mid){
        int cap = 0;
        int count = 1;
        for(int ele : nums){
            if(cap + ele <= mid){
    ...
    public int shipWithinDays(int[] weights, int days) {
        ...
        while(l <= h){
            ...
            if(countDays(weights,mid) > days){
                l=mid+1; //if no of days is more then increse capacity
            }
            else h=mid-1; // if no of days is less than or equal then decrease capacity
        }
        return l;
    }
